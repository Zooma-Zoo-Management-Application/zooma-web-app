"use client";

import { useState, useEffect } from "react";
import FormWrapper from "./FormWrapper";
import { Separator } from "@/components/ui/separator";
import { FormItems } from "@/app/tickets/page";

type StepProps = FormItems & {
  goTo: (index: number) => void;
};

const FinalStep = ({ date, tickets, goTo }: StepProps) => {
  const filteredTickets = tickets.filter((ticket) => ticket.quantity !== 0);

  const totalTicketsPrice = filteredTickets?.reduce(
    (acc, obj) => acc + obj.price,
    0
  );
  console.log(totalTicketsPrice);

  return (
    <FormWrapper
      title="Finishing Up"
      description="Double-check everything looks OK before confirming."
    >
      <div className="">
        <div className="bg-neutral-900 p-4 mt-2 rounded-md border border-neutral-700">
          <div className="flex justify-between items-center">
            <div className="">
              <h4 className="font-semibold md:text-lg">
                {/* {`${plan.charAt(0).toUpperCase() + plan.slice(1)} (${
                  yearly ? "Yearly" : "Monthly"
                })`} */}sdasasad
              </h4>
              <button
                onClick={() => goTo(1)}
                className="text-[#6fe79f] text-sm"
              >
                Change
              </button>
            </div>
            {/* <p className="font-semibold">{`$${
              yearly ? planPrice * 10 : planPrice
            }${yearly ? "/yr" : "/mo"}`}</p> */}
          </div>
          {filteredTickets.length > 0 && <Separator className="my-4" />}
          {filteredTickets?.map((ticket) => (
            <div
              className="flex justify-between items-center my-2"
              key={ticket.id}
            >
              <p className="text-neutral-400">{ticket.label}</p>
              {/* <p className="">{`$${yearly ? ticket.price * 10 : ticket.price}${
                yearly ? "/yr" : "/mo"
              }`}</p> */}
            </div>
          ))}
        </div>
        <div className="flex justify-between items-center my-4 px-4">
          <p className="text-neutral-400">
            Total 
            {/* (per {yearly ? "year" : "month"}) */}
          </p>
          <p className="text-[#6fe79f] font-semibold md:text-lg">
            +$
            {/* {yearly
              ? planPrice * 10 + totalAddOnsPrice * 10
              : planPrice + totalAddOnsPrice}
            /{yearly ? "yr" : "mo"} */}
          </p>
        </div>
      </div>
    </FormWrapper>
  );
};

export default FinalStep;